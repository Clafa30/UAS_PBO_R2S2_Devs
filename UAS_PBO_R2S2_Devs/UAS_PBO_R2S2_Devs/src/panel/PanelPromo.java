...
public class PanelPromo extends javax.swing.JPanel {
     ...
     */
    public PanelPromo(HalamanUtama halamanUtama, PanelRincian panelRincian1) { 
        ...
        spinnerBeefBurg.addChangeListener(e -> {
            ...
            updateCheckBoxFromSpinner(BBCbox, spinnerBeefBurg, "P001");  // Memperbarui checkbox dan mengelola keranjang
            updateTableFromSpinner("P001", spinnerBeefBurg);  // Memperbarui tabel dari spinner
        });
        ...
        spinnerChezBurg.addChangeListener(e -> {
            ...
            updateCheckBoxFromSpinner(CBCbox, spinnerChezBurg, "P002");  // Memperbarui checkbox dan mengelola keranjang
            updateTableFromSpinner("P002", spinnerChezBurg);  // Memperbarui tabel dari spinner
        });

        SpinFF.addChangeListener(e -> {
            int value = (Integer) SpinFF.getValue();
            if (value < 0) {
                SpinFF.setValue(0);  // Pastikan nilai tidak negatif
            }
            updateCheckBoxFromSpinner(FFCbox, SpinFF, "P003");  // Memperbarui checkbox dan mengelola keranjang
            updateTableFromSpinner("P003", SpinFF);  // Memperbarui tabel dari spinner
        });

        SpinnerCNA.addChangeListener(e -> {
            int value = (Integer) SpinnerCNA.getValue();
            if (value < 0) {
                SpinnerCNA.setValue(0);  // Pastikan nilai tidak negatif
            }
            updateCheckBoxFromSpinner(CNACbox, SpinnerCNA, "P004");  // Memperbarui checkbox dan mengelola keranjang
            updateTableFromSpinner("P004", SpinnerCNA);  // Memperbarui tabel dari spinner
        });

        SpinnerHD.addChangeListener(e -> {
            int value = (Integer) SpinnerHD.getValue();
            if (value < 0) {
                SpinnerHD.setValue(0);  // Pastikan nilai tidak negatif
            }
            updateCheckBoxFromSpinner(HDCbox, SpinnerHD, "P005");  // Memperbarui checkbox dan mengelola keranjang
            updateTableFromSpinner("P005", SpinnerHD);  // Memperbarui tabel dari spinner
        });
    ...
    // Panggil metode untuk memperbarui tabel di PanelRincian
    PanelRincian.getInstance(halamanUtama).refreshTable();
...
public void updateTableFromSpinner(String menuId, JSpinner spinner) {
    ...
    public void resetControls() {
        ...
        spinnerChezBurg.setValue(0);
        SpinFF.setValue(0);
        SpinnerCNA.setValue(0);
        SpinnerHD.setValue(0);

        // Reset checkbox ke tidak dicentang
        BBCbox.setSelected(false);
        CBCbox.setSelected(false);
        FFCbox.setSelected(false);
        CNACbox.setSelected(false);
        HDCbox.setSelected(false);
    }
